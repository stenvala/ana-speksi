...
2. **Read all functional specs**
   ...
   Read `bas-spec/ongoing/<name>/proposal.md` and all `functional-spec.md` files.
...
4. **Create research.md**
   ...
   At `bas-spec/ongoing/<name>/research.md`, document:
   - Relevant documentation under `bas-spec/truth`, note that this is recursive so identify the areas, if there are no areas, let user know that too so user can steer you if you didn't find anything
...
5. **Create technical-spec.md for each story**
   ...
   **Story selection**: If `/bs-continue` specified particular stories to
   ...
   For each story in `bas-spec/ongoing/<name>/specs/<NN-story>/`, create:
   - `technical-spec.md` -- implementation approach, architecture, relevant skills.
     Must have `**Status**: Draft` and `**Generated with**: bs-techify` in the header.
...
6. **Create data model change documents**
   ...
   Create `data-model.md` at `bas-spec/ongoing/<name>/data-model.md`
   (one per spec, not per story). This document must use the **same
   structure** as `bas-spec/truth/data-models/<domain>.md` so it can be
   ...
   Reference existing data models in `bas-spec/truth/data-models/` and
