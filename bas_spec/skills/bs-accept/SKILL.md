...
3. **Apply acceptance**
   ...
   ```
   uv run bas_spec accept
   ```

   (or with a spec name: `uv run bas_spec accept <name>`)
...
4. **Commit the acceptance (mandatory)**
   ...
   >
   > Include in the commit message that this is a bas-spec phase acceptance, e.g.:
   > `bas-spec: accept <phase> phase for <spec-name>`"
...
If any required file is not Accepted, `bs-continue` will block the transition
and direct the user to run `bs-accept` first.
