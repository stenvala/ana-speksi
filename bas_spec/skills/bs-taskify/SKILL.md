...
3. **Create implementation-order.md**
   ...
   (`bas-spec/ongoing/<name>/implementation-order.md`) using the template
   in `resources/implementation-order-template.md`.

   The file must contain:
   - `**Generated with**: bs-taskify` in the header
...
4. **Create tasks.md for each story**
   ...
   **Story selection**: If `/bs-continue` specified particular stories to
   ...
   - Reference the skill that guides the implementation via the
     ...
     decorative text -- during bs-codify, the agent MUST invoke each
