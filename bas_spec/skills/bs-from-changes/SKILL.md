...
3. **Create truth entries**
   ...
   in the `bas-spec/truth/` hierarchy. Group related changes into coherent
   features. Each feature gets its own folder under `bas-spec/truth/`.

   **For `--codebase` (whole codebase documentation):**

   When documenting an entire codebase, first create the `bas-spec/truth/platform/`
   ...
   **Enhancing existing platform documentation:**
   ...
   all content under `bas-spec/truth/platform/` based on current codebase state.
   ...
   - Data model references (link to `bas-spec/truth/data-models/` entries)
   ...
   in `bas-spec/truth/data-models/` using `resources/data-model-template.md`.
   ...
   `bas-spec/truth/enums/` using `resources/enum-template.md`.
...
- Data model entries must follow the `bas-spec/truth/data-models/` structure exactly
