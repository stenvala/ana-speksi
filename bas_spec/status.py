...
def get_bas_spec_root(cwd: Path | None = None) -> Path:
    """Return the bas-spec root directory, searching upward from cwd."""
    start = cwd or Path.cwd()
    current = start
    while current != current.parent:
        candidate = current / BAS_SPEC_DIR
        if candidate.is_dir():
            return candidate
        current = current.parent
    return start / BAS_SPEC_DIR


def ensure_dirs(root: Path) -> None:
    """Ensure all required subdirectories exist."""
    from bas_spec.models import TRUTH_DATA_MODELS_DIR, TRUTH_ENUMS_DIR
    ...
    for truth_sub in [TRUTH_DATA_MODELS_DIR, TRUTH_ENUMS_DIR]:
        (root / TRUTH_DIR / truth_sub).mkdir(parents=True, exist_ok=True)
