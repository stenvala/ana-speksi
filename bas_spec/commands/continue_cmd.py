"""bas_spec bs-continue -- advance a spec to the next phase."""
...
from bas_spec.models import Phase, PHASE_DESCRIPTIONS, SpecStatus
from bas_spec.commands.accept import get_acceptance_status
from bas_spec.status import (
    get_bas_spec_root,
    list_ongoing_specs,
    get_spec_status,
    print_status,
...
def continue_command(
    ...
    if not specs:
        console.print("[yellow]No ongoing specs found. Run bs-new first.[/yellow]")
        raise typer.Exit(1)
    ...
    if acceptance["files_to_accept"]:
        ...
        for f in acceptance["files_to_accept"]:
            console.print(f"  - {f}")
        console.print("\nRun [bold cyan]bs-accept[/bold cyan] to accept them first.")
        return
...
def _phase_to_skill(phase: Phase) -> str:
    ...
    mapping = {
        Phase.PROPOSAL: "bs-new",
        Phase.STORIFY: "bs-storify",
        Phase.RESEARCH: "bs-techify",
        Phase.TECHIFY: "bs-techify",
        Phase.TASKIFY: "bs-taskify",
        Phase.CODIFY: "bs-codify",
        Phase.DOCUFY: "bs-docufy",
    }
    return mapping.get(phase, "bs-continue")
