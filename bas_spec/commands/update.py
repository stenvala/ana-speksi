"""bas_spec update -- regenerate skills and commands without touching bas-spec/."""
...
from bas_spec.models import AgentFramework, BAS_SPEC_DIR
...
def update_command(
    ...
    """Update agent skills and commands without touching the bas-spec/ folder.

    Regenerates all skill files and command/prompt files for the selected
    agent frameworks based on the latest bas-spec skill definitions and
    project config. The bas-spec/ directory (config, ongoing, truth, archive)
    is left untouched.
    """
    config_path = project_dir / BAS_SPEC_DIR / "config.yaml"
    if not config_path.exists():
        console.print(
            "[red]bas-spec is not initialized in this project. Run 'bas_spec init' first.[/red]"
        )
        raise typer.Exit(1)
    ...
    if not frameworks:
        ...
        if detected:
            ...
            console.print(
                ...
                "Specify frameworks with --framework or run 'bas_spec init'.[/red]"
            )
            raise typer.Exit(1)
